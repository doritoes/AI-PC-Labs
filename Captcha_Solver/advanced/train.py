...
def train():
    ...
    print(f"📡 Stage 1: {STAGE_1_EPOCHS} Epochs (Digits Only)")
    print(f"📡 Stage 2: {STAGE_2_EPOCHS} Epochs (Full Alphanum + 30% Hybrid)")
    print(f"📦 Image Size: {config.WIDTH}x{config.HEIGHT} | Batch Size: {BATCH_SIZE}")
    print("="*60)
    ...
    for epoch in range(1, STAGE_1_EPOCHS + STAGE_2_EPOCHS + 1):
        ...
        for batch_idx, (imgs, target) in enumerate(dataloader):
            ...
            if batch_idx % 5 == 0 or batch_idx == num_batches - 1:
                ...
                print(f"\r  ⚡ [Ep {epoch:02d}] Progress: {progress:3.0f}% | Acc: {current_acc:5.1f}% | Loss: {loss.item():.4f} ", end="")

        avg_loss = epoch_loss / num_batches
        final_acc = (correct / total) * 100
        
        phase_name = "DIGITS" if epoch <= STAGE_1_EPOCHS else "HYBRID"
        print(f"\n✅ Epoch [{epoch:02d}] COMPLETE | Phase: {phase_name}")
        print(f"   Avg Loss: {avg_loss:.4f} | Final Acc: {final_acc:.2f}% | Time: {time.time()-start_time:.1f}s")
